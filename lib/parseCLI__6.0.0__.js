...
exports.babelCoreOpts = exports.babelPluginTransformRuntimeOpts = exports.babelPresetEnvOpts = exports.babelMultiEnvOpts = exports.filenames = void 0;
...
(function enableChalkCommanderOptionFlags(scoped) {
  ...
  const Command = _commander.default.Command;
  const nativeOptionFn = Command.prototype.option;

  Command.prototype.option = function(...args) {
    const flags = args[0];
    args[0] = flags.replace(/{\S+ (.+)}/, "$1");
    const returnValue = nativeOptionFn.apply(this, args); // https://github.com/tj/commander.js/blob/master/index.js#L39
...
const filenames = _lodash.default.flowRight(
  _lodash.default.uniq,
  _lodash.default.partial(
    _lodash.default.reduce,
    _lodash.default,
    (globbed, input) => {
...
const babelPluginTransformRuntimeOpts = _lodash.default.pick(opts, [
  "helpers",
  "polyfill",
  "regenerator",
  "moduleName"
...
const babelCoreOpts = _lodash.default.omit(opts, [
  "useBuiltInsUsage",
  ..._lodash.default.keys(babelMultiEnvOpts),
  ..._lodash.default.keys(babelPresetEnvOpts),
...
if (!_lodash.default.isEmpty(errors)) {
  errors.forEach(it =>
    console.error(_chalk.default`{red ${_figures.default.cross} } ${it}`)
  );
  process.exit(2);
}
